...
export class BorrowedService {
  ...
  async createBorrowedFromRequest(requestId: string, returnDays: number = 14): Promise<Borrowed> {
  ...
  async getAllBorrowed(): Promise<Borrowed[]> {
    return this.borrowedModel.find()
  ...
  async getActiveBorrowings(): Promise<Borrowed[]> {
    return this.borrowedModel.find({ returned: false })
      .populate('userId', 'name email')
      .populate('bookId', 'bookTitle author')
      .exec();
  }
  ...
  async getUserBorrowings(userId: string): Promise<Borrowed[]> {
    return this.borrowedModel.find({ userId })
      .populate('bookId', 'bookTitle author')
      .exec();
  }
  ...
  async returnBook(borrowedId: string): Promise<Borrowed> {
    ...
    if (book) {
      ...
      if (book.borrowers) {
        book.borrowers = book.borrowers.filter(id => id.toString() !== borrowedId);
      }
      ...
      if (book.borrowers.length === 0) {
        ...
        const activeBorrowings = await this.borrowedModel.find({
          bookId: book._id,
          returned: false
        }).sort({ returnDate: 1 }).limit(1);
  ...
  async getBorrowedById(id: string): Promise<Borrowed> {
    const borrowed = await this.borrowedModel.findById(id)
      .populate('userId', 'name email')
      .populate('bookId', 'bookTitle author');
